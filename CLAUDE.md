...
| 10 | `code/` scaffold | ✅ **done, committed.** 🔴 **Two artifacts, deliberately:** `code/spikes/` is **LIVE** (U12 · U21-a · U27 · U30 spikes) · `code/extension/` is **TEAM-TEST + Slice 2 in progress** · `code/backend/` is **LIVE (extract/redact only)** — doc 01 §2's shape. **The U12/U27 spikes are raw MV3, NOT WXT** (doc 01 §6's eject rule). **Read [`code/README.md`](code/README.md) §Order of operations before building anything.** |
...
| 12 | `code/` **Slice 2** — **file-content checking** | 🔴 **IN PROGRESS** — locked plan: [`docs/superpowers/plans/2026-07-18-slice-2-file-content.md`](docs/superpowers/plans/2026-07-18-slice-2-file-content.md). **Two stop conditions before build (same discipline as U12):** **U27** attach interception ✅ · **U30** PDF redaction 🟠 (smoke PASS; real corpus + ship gate still owed). **ADR 0027 (amended):** extract is a decision surface; masks apply to the **original file** — not `.txt` by default. **ADR 0028:** backend parses, extension detects. Live acceptance: [`code/extension/ACCEPTANCE.md`](code/extension/ACCEPTANCE.md). B3 does NOT go between the slices. |
| 13 | `ml/` **sensitive-vs-not** — **PARALLEL team track** | 🟢 **Runs alongside Slices 1–2, does NOT block them** ([ADR 0018](docs/adr/0018-sensitive-vs-not-parallel-track.md)). **Separate team, `ml/` sibling of `code/`.** Brief: [`docs/team/sensitive-vs-not-parallel-track.md`](docs/team/sensitive-vs-not-parallel-track.md). 🔴 **LOCKED plan (2026-07-18, accepted): [`docs/superpowers/plans/2026-07-18-sensitive-vs-not-ml.md`](docs/superpowers/plans/2026-07-18-sensitive-vs-not-ml.md)** — a **span classifier over stock-NER-proposed PER/ORG spans** (architecture B, [ADR 0019](docs/adr/0019-sensitivity-span-classifier-over-ner.md)); **supersedes** the 2026-07-17 parallel-track plan. Decisions recorded in **ADRs [0019](docs/adr/0019-sensitivity-span-classifier-over-ner.md)–[0023](docs/adr/0023-ml-compute-and-residency-gates.md)**. **`ml/` is NOT yet scaffolded — execution happens in Cursor** (subagent-driven, task-by-task with human gates). **Integrates into the extension AFTER Slice 2.** **L1 keeps sole ownership of NRIC/SSM/TIN digits.** ⚠️ **This-phase eval substrate is `human_simulated` (ADR 0022) — a founder waiver that does NOT discharge ADR 0015's real-substrate requirement for a PRODUCTION ship; synthetic-only or always-KEEP = NOT_SHIPPED (ADR 0021).** ⚠️ **Not the CTO's deliverable — this row tracks a track the founder owns and staffs.** |
...
decision that puts real personal data in the company) · 🔴 **0016 MVP-first sequencing** (**reverses *"B3 above the engineering spikes"*** — the team test is the next learning loop; B3, force-install, U6-b's threshold, marketing and **doc 08** are **parked** until Slices 1 and 2 land). · **0017 Slice 1's four technical choices** (stock L2 · CDN weights · WXT with committed dist · block+Ignore) · 🔴 **0018 sensitive-vs-not is a PARALLEL track** (in , integrated AFTER Slice 2; sensitivity does NOT gate files; L1 keeps sole ownership of identifier digits). · **0019 sensitivity = span classifier over NER proposals** (not a standalone MASK tagger; integrated recall NER-bounded) · **0020 mDeBERTa-v3-base baseline** (xlm-roberta-base forbidden; on-device size eng-gated; distillation rides doc 06 §6.2) · **0021 three-way provenance + structural ship gate** (`llm_synthetic`/`human_simulated`/`real`; synthetic-only or always-KEEP = NOT_SHIPPED; numeric threshold human-gated) · **0022 `human_simulated` substrate + counsel-STOP waiver** (privacy-clean; re-armed if `real` data enters; does NOT discharge ADR 0015 for production) · **0023 ML compute/residency gates** (CPU-only CI; Colab for synthetic; `real` stays local-MY; don't-train-on-the-exam). **These five (0019–0023) cover the parallel ML track; `ml/` is not yet scaffolded — executed in Cursor.** · **0024** L1 composer hints · **0025** Send-time per-span review · **0026 Report false detection after Slice 2** (opt-in; ≠ Ignore; admin-gated; not Slice 2 MVP) · **0027 format-preserving file redaction — extract is decision surface, not output** (amended 2026-07-18; rejects `.txt`-on-accept default) · **0028 backend parses, extension detects** (Pushback 2). New ADRs continue from **0029**.
...
> | **3** | **Slice 2 — file CONTENT checking** | **In progress** — [`docs/superpowers/plans/2026-07-18-slice-2-file-content.md`](docs/superpowers/plans/2026-07-18-slice-2-file-content.md). 🔴 **Two stop conditions (same shape as U12 for Slice 1):** **U27** attach interception ✅ (spike PASS 2026-07-18) · **U30** PDF format-preserving redaction 🟠 (smoke PASS; **real corpus still owed**; commercial ship gate unchanged). **Binding:** [ADR 0027](docs/adr/0027-cleaned-extract-replaces-attachment.md) **amended form** — masks on the original file; extract is a decision surface only. [ADR 0028](docs/adr/0028-backend-parses-extension-detects.md): parse in cloud, detect on device. Live checklist: [`code/extension/ACCEPTANCE.md`](code/extension/ACCEPTANCE.md). B3 does NOT go between the slices. |
